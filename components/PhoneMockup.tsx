...
function Parcel3D() {
  ...
  // Colores
  const boxColor: THREE.ColorRepresentation = '#E2B887'; // cartón
  const tapeColor: THREE.ColorRepresentation = '#B8893C'; // cinta

  return (
    <Float speed={0.8} rotationIntensity={0.2} floatIntensity={0.5}>
      {/* ↓↓↓ Bajamos todo el grupo 3D un poco */}
...
export default function PhoneMockup() {
  ...
  useEffect(() => {
    ...
    // Secuencia
    tl.to(b1.current, { autoAlpha: 1, y: 0, duration: 0.5 })
      .to(typing.current, { autoAlpha: 1, duration: 0.2 }, '+=0.1')
      .to(typing.current, { autoAlpha: 0, duration: 0.2 }, '+=0.9')
      .to(b2.current, { autoAlpha: 1, y: 0, duration: 0.5 })
      .to(ticks2.current, { scale: 1, duration: 0.25 }, '-=0.1')
      .to(b3.current, { autoAlpha: 1, y: 0, duration: 0.5 }, '+=0.2')
  ...
  return (
    <div className="mx-auto w-72 md:w-96 select-none">
      {/* Sombra debajo */}
      <div className="mx-auto h-8 w-2/3 rounded-full blur-2xl opacity-40 bg-black/30" aria-hidden />

      {/* Marco del teléfono (agregamos 'group' para el glare hover) */}
      <div
        className="
          group relative aspect-[9/19.5] w-full
          rounded-[42px] border border-slate-300/60 dark:border-slate-700
      ...
      >
        {/* Botones laterales */}
        <div className="absolute -left-0.5 top-16 h-8 w-1 rounded-r-lg bg-slate-400/70 dark:bg-slate-600" aria-hidden />
        <div className="absolute -left-0.5 top-28 h-10 w-1 rounded-r-lg bg-slate-400/70 dark:bg-slate-600" aria-hidden />
        <div className="absolute -right-0.5 top-24 h-12 w-1 rounded-l-lg bg-slate-400/70 dark:bg-slate-600" aria-hidden />

        {/* Pantalla */}
        <div className="absolute inset-1.5 rounded-[36px] overflow-hidden bg-[linear-gradient(135deg,#f8fafc_0%,#eef2ff_40%,#e0f2fe_100%)] dark:bg-[linear-gradient(135deg,#0b1220_0%,#0b1a2b_40%,#0b2030_100%)]">
          {/* Notch */}
          <div className="absolute top-1 left-1/2 -translate-x-1/2 h-6 w-28 rounded-b-2xl bg-black/80" aria-hidden>
            <div className="absolute left-3 top-1 h-2 w-6 rounded-full bg-black/30" />
            <div className="absolute right-3 top-1 h-2 w-2 rounded-full bg-black/30" />
          </div>

          {/* Reflections / Glass shimmer */}
          <div
            className="pointer-events-none absolute -left-16 -top-10 h-[150%] w-40 rotate-[25deg] opacity-20"
            style={{
              background:
            ...
            aria-hidden
          />
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(120%_60%_at_10%_0%,rgba(255,255,255,0.55),transparent_50%)] dark:bg-[radial-gradient(120%_60%_at_10%_0%,rgba(255,255,255,0.08),transparent_50%)]" aria-hidden />

          {/* 3D detrás del chat - lo empujamos hacia abajo */}
          <div className="absolute inset-x-0 top-12 bottom-0 pointer-events-none -z-0" aria-hidden>
            <Canvas dpr={[1, 2]}>
              <ambientLight intensity={0.5} />
              <directionalLight position={[2, 4, 3]} intensity={1.2} castShadow />
              <Parcel3D />
              {/* "Suelo" suave (también lo bajamos un poco más) */}
          ...
          {/* Barra de estado */}
          <div className="relative z-10 flex items-center justify-between px-4 pt-8 text-[11px] text-slate-700 dark:text-slate-300">
          ...
          {/* Chat (más padding abajo para que nada lo tape) */}
          <div ref={ctn} className="relative z-10 p-4 pb-10 flex flex-col gap-2 text-[13px] leading-snug">
            {/* BURBUJA 1 (user) */}
            <div
              ref={b1}
              className="self-start max-w-[78%] rounded-2xl bg-white/80 dark:bg-white/10 border border-black/5 dark:border-white/10 backdrop-blur px-3 py-2 text-slate-700 dark:text-slate-200 shadow-sm"
            >
            ...
            <div
              ref={typing}
              className="self-end max-w-[78%] rounded-2xl bg-[#0A84FF]/15 text-slate-900 dark:text-white px-3 py-2 border border-[#0A84FF]/30"
            >
              <TypingDots />
            </div>
            ...
            <div
              ref={b2}
              className="self-end max-w-[78%] rounded-2xl bg-[#0A84FF]/20 text-slate-900 dark:text-white px-3 py-2 border border-[#0A84FF]/30 shadow-sm"
            >
            ...
            <div
              ref={b4}
              className="self-end max-w-[78%] rounded-2xl bg-[#FF6B00]/15 border border-[#FF6B00]/30 px-3 py-2 text-slate-900 dark:text-white shadow-sm"
            >
        ...
        {/* Glass glare animado */}
        <div
          className="pointer-events-none absolute -left-16 top-0 h-full w-28 rotate-[18deg] opacity-0 group-hover:opacity-20"
          style={{
            background:
